...
public class Bridge2 {
    public int solution(int[] stones, int k) {
        ...
        while (start <= last) {
            ...
            if (isCanPass(mid, k, stones)) {
                start = mid;
            } else {
                last = mid;
            }
            if (prevLast == last && prevStart == start) {
                break;
            }
        }
        return start;
    ...
    private boolean isCanPass(int memberCount, int k, int[] stones) {
        int noUseStoneCount = 0;
        for (int i = 0; i < stones.length; i++) {
            if (stones[i] < memberCount) {
                noUseStoneCount++;
            } else {
                noUseStoneCount = 0;
            }
            if (noUseStoneCount >= k) {
                return false;
            }
        }
        return true;

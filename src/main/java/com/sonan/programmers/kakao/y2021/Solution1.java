...
public class Solution1 {
    private Set<Character> valid = new HashSet(){{
       for (int i = 'a'; i <= 'z'; i++) {
           add((char)i);
       }
        for (int i = '0'; i <= '9'; i++) {
    ...
    public String solution(String new_id) {
        char[] new_id1 = new_id.toLowerCase().toCharArray();

        StringBuffer new_id2 = new StringBuffer();
        for (int i = 0; i < new_id1.length; i++) {
            if (valid.contains(new_id1[i])) {
                new_id2.append(new_id1[i]);
            }
        }
        char[] new_id2_char = new_id2.toString().toCharArray();
        StringBuffer new_id3 = new StringBuffer();
        int dotIndex = -1;
        for (int i = 0; i < new_id2_char.length; i++) {
            if (new_id2_char[i] == '.') {
                dotIndex = i;
            } else {
                if (dotIndex != -1) {
                ...
                new_id3.append(new_id2_char[i]);
            }
        }

        //first, last deleted. already
        String new_id4 = new_id3.toString();
        ...
        if (new_id5.length() >= 15) {
            ...
            if (new_id6.toCharArray()[14] == '.') {
                new_id6 = new_id6.substring(0, 14);
            }
        }

        StringBuffer new_id7 = new StringBuffer();
        if (new_id6.length() <= 2) {
            new_id7.append(new_id6.substring(0, new_id6.length()-1));
            while (new_id7.length() != 3) {
                new_id7.append(new_id6.charAt(new_id6.length()-1));
            }
        } else {
            new_id7.append(new_id6);
        }
        return new_id7.toString();
    }
}

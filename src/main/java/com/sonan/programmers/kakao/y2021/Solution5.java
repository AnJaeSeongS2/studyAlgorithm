...
//        "50:00:00"	"50:00:00"	["15:36:51-38:21:49", "10:14:18-15:36:51", "38:21:49-42:51:45"]	"00:00:00"
    public String solution(String play_time, String adv_time, String[] logs) {
        Integer play = genTimeToInteger(play_time);
        Integer adv = genTimeToInteger(adv_time);
        int[] count = new int[360001];
        for (int i = 0; i < logs.length; i++) {
            String[] logEntry = logs[i].split("-");
            Integer start = genTimeToInteger(logEntry[0]);
            Integer end = genTimeToInteger(logEntry[1]);
        ...
        for (int i = 0; i <= play - adv; i++) {
            ...
            if (prevCount == -1) {
                ...
                prevCount = prevCount - count[i-1] + count[i + adv];
                curMax = prevCount;
            }
            if (curMax > max) {
                max = curMax;
        ...
        return genIntegerToTimeString(resultStartIndex);
    }

    private Integer genTimeToInteger(String time) {
        String[] pt = time.split(":");
        return Integer.valueOf(pt[0])*3600 + Integer.valueOf(pt[1])*60 + Integer.valueOf(pt[2]);
    }

    private String genIntegerToTimeString(Integer time) {

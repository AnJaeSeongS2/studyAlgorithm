...
public class Sol2 {
    public String[] solution(String s) {
        ...
        while (!curStr.isEmpty()) {
            String piece = genPiece(curStr);
            if (piece.length() == curStr.length()) {
                // end case; xyzd
                curStr = "";
                isOddEnd = true;
            } else {
                curStr = curStr.substring(piece.length(), curStr.length() - piece.length());
            }
            result.add(piece);
        }

        for (int i = isOddEnd ? result.size()-2 : result.size()-1; i >= 0; i--) {
    ...
    private String genPiece(String s) {
